...
def evaluate(model, loader, criterion, device, class_names=None, save_plot_path="per_class_accuracy.png"):
    """
    Evaluate model on a dataset, return loss, Top-1/Top-5 accuracy, and per-class accuracy.
    """
    model.eval()
    ...
    with torch.no_grad():
        for inputs, labels in loader:
            ...
            # Per-class accuracy
            _, pred_top1 = preds.topk(1, dim=1)
    ...
    print(f"Loss: {val_loss:.4f} | Top-1 Acc: {top1_acc:.2f}% | Top-5 Acc: {top5_acc:.2f}%")
...
def main(args):
    ...
    # Load test dataloader (train, val, test all available)
    ...
    writer.add_scalar("Test/Top1_Accuracy", top1_acc, 0)
    writer.add_scalar("Test/Top5_Accuracy", top5_acc, 0)
    writer.close()
